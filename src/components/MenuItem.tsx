...
export function Item({title,price,serving,sub,desc = "Description"}: {title:string, price:string, serving?:string, sub?:boolean, desc:string}) {
    const [isOpen, setIsOpen] = useState(false)
    const toggleOpen = () => {
        setIsOpen(!isOpen)
    }
    return (
        <div className={`w-full flex flex-col ${sub ? 
            "pl-64" : ""
        }`}>
            <button onClick={toggleOpen} className="flex gap-32 items-center">
                <h4 className="whitespace-nowrap text-[32px]">{title}</h4>
                <div className="h-[1px] w-full bg-black/25"></div>
                <h4 className="text-black/75 whitespace-nowrap  text-[32px]">{serving}</h4>
                <h4 className="text-brand-dark text-[32px]">{price}</h4>
            </button>
